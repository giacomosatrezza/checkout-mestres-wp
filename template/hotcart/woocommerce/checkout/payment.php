...
defined( 'ABSPATH' ) || exit;
	...
	$('label[for=\"'+selected_Id+'\"] .active').removeClass('hide');
			...
			$('.cwmp_woo_cart .title .woocommerce-Price-amount').html('".wp_kses_post(wc_price(WC()->cart->total))."');
			$('.cwmp_woo_cart .title h2 span').html('(".esc_html(WC()->cart->get_cart_contents_count()).")');
		});
		</script>
	";
...
<div class="woocommerce-checkout-payment">
	...
	<div class="cwmp_box_method_shipping" style="margin-bottom:20px;">
		<h2>
		   <i class="fa <?php echo esc_html(get_option('cwmp_checkout_box_icon_frete')); ?>"></i>
	...
	<?php if(get_option('cwmp_activate_order_bump')=="S"){ ?>
						...
						if($i==1){
							$product2 = json_decode($cart_item2['data']);
								...
								<div class="cart-item bump">
									...
									<div class="product-thumbnail"><a href="javascript:void(0)" id='<?php echo esc_html($get_product_bump->get_id()); ?>' class='cwmp_add_order_bump'><?php echo wp_kses_post($get_product_bump->get_image()); ?></a></div>
									<div class="product-name">
										<div class="bump-chamada"><?php echo esc_html($product_bump->chamada); ?></div>
										<a href="javascript:void(0)" id='<?php echo esc_html($get_product_bump->get_id()); ?>' class='cwmp_add_order_bump'><h3><?php echo esc_html($get_product_bump->get_name()); ?></h3></a>
										<?php
										$discount = ( $get_product_bump->get_price() * $product_bump->valor ) / 100;
										echo "<span class='woocommerce-Price-amount amount'><s>de ".esc_html(wp_strip_all_tags(wc_price($get_product_bump->get_price())))."</s> por ".esc_html(wp_strip_all_tags(wc_price($get_product_bump->get_price()-$discount)))."</span>";
										?>
									</div>
									<div class="bump-button">
									<?php echo "<a href='javascript:void(0)' id='".esc_html($get_product_bump->get_id())."' class='cwmp_add_order_bump'>".esc_html__( 'Add to Cart', 'checkout-mestres-wp' )."</a>"; ?>
									<?php echo "<a href='javascript:void(0)' class='cwmp_not_order_bump'>".esc_html__( 'No, Thanks', 'checkout-mestres-wp' )."</a>"; ?>
			...
			<div class="cwmp_woo_form_payment">
				<h2>
					<i class="fa <?php echo esc_html(get_option('cwmp_checkout_box_icon_pagamento')); ?>"></i>
	...
	<div class="form-row place-order">
		<noscript>
			<?php
				printf(
					// Translators: %1$s and %2$s are HTML <em> tags used to emphasize the "Update Totals" text.
					esc_html__(
